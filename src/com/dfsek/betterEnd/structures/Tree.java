		...
		for(int i = 0; i < length; i++) {
			...
			if(lvl >= 1) {
				for (int x = -5; x <= 5; x++) {
					for (int y = -5; y <= 5; y++) {
						for (int z = -5; z <= 5; z++) {
							if(start.clone().add(x,y,z).getBlock().isPassable() && start.clone().add(x,y,z).distance(start) < 2.5 && start.clone().add(x,y,z).getBlock().isEmpty()) start.clone().add(x,y,z).getBlock().setType(l);
						}
					}
				}
			}
