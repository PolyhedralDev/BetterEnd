		...
		if(totalChunkDistance2D > 50 || ConfigUtil.allAether) {
			for(int X = 0; X < 16; X++)
				for(int Z = 0; Z < 16; Z++) {
					...
					double heatNoiseLvl = biomeGenerator.noise((double) (chunkX * 16 + X) / ConfigUtil.heatNoise, (double) (chunkZ * 16 + Z) / ConfigUtil.heatNoise, 0.5D,
							0.5D);
					double totalDistance2D = (chunkX * 16 + X > 1250 || chunkZ * 16 + Z > 1250)
							? 2000
							: Math.sqrt(Math.pow(((double) chunkX * 16) + X, 2) + Math.pow(((double) chunkZ * 16) + Z, 2));
					...
					if((biomeNoiseLvl > 0.45 || ConfigUtil.allAether) && ConfigUtil.doClouds) {
						...
						int yMinc2 = 255;
						int yMaxc2 = 0;
						double c1Noise = generator.noise((double) (chunkX * 16 + X) / ConfigUtil.cloudNoise, (double) (chunkZ * 16 + Z) / ConfigUtil.cloudNoise, 0.1D,
								0.55D);
						double c2Noise = generator.noise((double) ((chunkX * 16 + X) - 1000) / ConfigUtil.cloudNoise,
								(double) ((chunkZ * 16 + Z) - 1000) / ConfigUtil.cloudNoise, 0.1D, 0.55D);
					...
					int height = (int) (generator.noise((double) (chunkX * 16 + X) / 24, (double) (chunkZ * 16 + Z) / 24, 0.5D, 0.7D) - ConfigUtil.landPercent)
							+ ConfigUtil.islandHeight;
					int biomeNoise = 0;
					if(biomeNoiseLvl < 0) {
						biomeNoise = (int) (biomeNoiseLvl * 2.5
								* (generator.noise((double) (chunkX * 16 + X) / 12, (double) (chunkZ * 16 + Z) / 12, 0.5D, 0.7D) - ConfigUtil.landPercent));
						height = (int) (biomeNoiseLvl * 4
								* (generator.noise((double) (chunkX * 16 + X) / 16, (double) (chunkZ * 16 + Z) / 16, 0.5D, 0.7D) - ConfigUtil.landPercent))
								+ ConfigUtil.islandHeight;
					}
					int yMin = 255;
					int yMax = 0;
					...
					double iNoise = generator.noise((double) (chunkX * 16 + X) / ConfigUtil.outerEndNoise, (double) (chunkZ * 16 + Z) / ConfigUtil.outerEndNoise, 0.1D,
							0.55D);
					if(totalDistance2D < 1050 && totalDistance2D > 975) {
						double noiseM = (-Math.pow(1.1, -totalDistance2D + 1000)) + 1;
						if(noiseM > 1) noiseM = 1;
					...
					for(int Y = yMax; Y > yMin; Y--) {
						if(Math.abs(generator.noise((double) (chunkX * 16 + X) / 12D, (double) Y / 12D, (double) (chunkZ * 16 + Z) / 12D, 0.5D, 0.5D)) < 0.8) {
							...
							if(biomeNoiseLvl > 0.5) {
									...
									if(upOne == Material.AIR) {
										...
										if(heatNoiseLvl < -0.5 && random.nextInt(100) < -75 * (heatNoiseLvl + 0.5) && chunk.getBlockData(X, Y, Z).getMaterial() != Material.AIR) {
											...
											if(plant == Material.TALL_GRASS) {
												...
												if(heatNoiseLvl < -0.5 && (biomeNoiseLvl > 0.5 || ConfigUtil.allAether)) chunk.setBlock(X, Y + 1, Z,
														main.getServer().createBlockData("minecraft:sweet_berry_bush[age=" + (random.nextInt(2) + 2) + "]"));
											} else {
												if((plant != Material.WITHER_ROSE)
														|| (heatNoiseLvl < -0.5 && (biomeNoiseLvl > 0.5 || ConfigUtil.allAether))) chunk.setBlock(X, Y + 1, Z, plant);
											}
										}
										...
										switch(random.nextInt(7)) {
											...
											case 5:
												switch(random.nextInt(2)) {
													case 0:
														if(random.nextInt(100) < 50) chunk.setBlock(X, Y + 1, Z,
																main.getServer().createBlockData("minecraft:stone_button[face=floor,facing=north]"));
														break;
													case 1:
														if(random.nextInt(100) < 50) chunk.setBlock(X, Y + 1, Z,
																main.getServer().createBlockData("minecraft:stone_button[face=floor,facing=east]"));
														break;
													default:
												}
												break;
								...
								if(Y < yMax - 1 && random.nextInt(100) < 32 && upOne == Material.AIR && chunk.getBlockData(X, Y, Z).getMaterial() == Material.END_STONE
