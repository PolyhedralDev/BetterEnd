...
public class EndChunkGenerator extends ChunkGenerator {
	...
	public ChunkData generateChunkData(World world, Random random, int chunkX, int chunkZ, BiomeGrid biome) {
		...
		if(totalChunkDistance2D > 50 || ConfigUtil.allAether) {
			for(int X = 0; X < 16; X++)
				for(int Z = 0; Z < 16; Z++) {
					double biomeNoiseLvl = (ConfigUtil.allAether) ? 1 : biomeGenerator.noise((double) (chunkX*16+X)/ConfigUtil.biomeSize, (double) (chunkZ*16+Z)/ConfigUtil.biomeSize, 0.5D, 0.5D);
					double heatNoiseLvl = biomeGenerator.noise((double) (chunkX*16+X)/ConfigUtil.heatNoise, (double) (chunkZ*16+Z)/ConfigUtil.heatNoise, 0.5D, 0.5D);
					double totalDistance2D = (chunkX*16+X > 1250 || chunkZ*16+Z > 1250)  ? 2000 : Math.sqrt(Math.pow(((double) chunkX*16)+X, 2)+Math.pow(((double) chunkZ*16)+Z, 2));
					...
					if((biomeNoiseLvl > 0.45 || ConfigUtil.allAether) && ConfigUtil.doClouds) {
						double aetherLvl = -Math.pow(256, -biomeNoiseLvl+0.45)+1;
						if(aetherLvl < 0) aetherLvl = 0;
						if(ConfigUtil.allAether) aetherLvl = 1;
						int yMinc = 255;
						int yMaxc = 0;
						int yMinc2 = 255;
						int yMaxc2 = 0;
						double c1Noise = generator.noise((double) (chunkX*16+X)/ConfigUtil.cloudNoise, (double) (chunkZ*16+Z)/ConfigUtil.cloudNoise, 0.1D, 0.55D);
						double c2Noise = generator.noise((double) ((chunkX*16+X)-1000)/ConfigUtil.cloudNoise, (double) ((chunkZ*16+Z)-1000)/ConfigUtil.cloudNoise, 0.1D, 0.55D);
						if(totalDistance2D < 1050 && totalDistance2D > 975) {
							...
							if(ConfigUtil.allAether) noiseM = 1;
							yMinc = (int) ((-5*noiseM*aetherLvl*(c1Noise-0.2)+ConfigUtil.cloudHeight)+1);
							yMaxc = (int) (5*noiseM*aetherLvl*(c1Noise-0.2)+ConfigUtil.cloudHeight-1);
							yMinc2 = (int) ((-4*noiseM*aetherLvl*(c2Noise-0.125)+ConfigUtil.cloudHeight+8)+1);
							yMaxc2 = (int) (4*noiseM*aetherLvl*(c2Noise-0.125)+ConfigUtil.cloudHeight+7);
						} else if(totalDistance2D > 1050) {
							yMinc = (int) (-5*aetherLvl*(c1Noise-0.15)+ConfigUtil.cloudHeight)+1;
							yMaxc = (int) (5*aetherLvl*(c1Noise-0.15)+ConfigUtil.cloudHeight-1);
							yMinc2 = (int) (-4*aetherLvl*(c2Noise-0.125)+ConfigUtil.cloudHeight+8)+1;
							yMaxc2 = (int) (4*aetherLvl*(c2Noise-0.125)+ConfigUtil.cloudHeight+7);
						}
					...
					if(ConfigUtil.allAether) totalDistance2D = 2000;
					int height = (int) (generator.noise((double) (chunkX*16+X)/24, (double) (chunkZ*16+Z)/24, 0.5D, 0.7D)-ConfigUtil.landPercent)+ConfigUtil.islandHeight;
					int biomeNoise = 0;
					if(biomeNoiseLvl < 0) {
						biomeNoise = (int) (biomeNoiseLvl*2.5*(generator.noise((double) (chunkX*16+X)/12, (double) (chunkZ*16+Z)/12, 0.5D, 0.7D)-ConfigUtil.landPercent));
						height = (int) (biomeNoiseLvl*4*(generator.noise((double) (chunkX*16+X)/16, (double) (chunkZ*16+Z)/16, 0.5D, 0.7D)-ConfigUtil.landPercent))+ConfigUtil.islandHeight;
					}
					int yMin = 255;
					int yMax = 0;
					double voidM = 1;
					...
					double iNoise = generator.noise((double) (chunkX*16+X)/ConfigUtil.outerEndNoise, (double) (chunkZ*16+Z)/ConfigUtil.outerEndNoise, 0.1D, 0.55D);
					if(totalDistance2D < 1050 && totalDistance2D > 975) {
						double noiseM = (-Math.pow(1.1, -totalDistance2D+1000))+1;
						if(noiseM > 1) noiseM = 1;
						if(noiseM < 0) noiseM = 0;
						yMin = (int) ((-ConfigUtil.islandHeightMultiplierBottom*voidM*noiseM*(iNoise-ConfigUtil.landPercent)+height+biomeNoise)+1);
						yMax = (int) (ConfigUtil.islandHeightMultiplierTop*voidM*noiseM*(iNoise-ConfigUtil.landPercent)+height-biomeNoise);
					} else if(totalDistance2D > 1050) {
						yMin = (int) (-ConfigUtil.islandHeightMultiplierBottom*voidM*(iNoise-ConfigUtil.landPercent)+height+biomeNoise)+1;
						yMax = (int) (ConfigUtil.islandHeightMultiplierTop*voidM*(iNoise-ConfigUtil.landPercent)+height-biomeNoise);
					}
					...
					for (int Y = yMax; Y > yMin; Y--) {
						if(Math.abs(generator.noise((double) (chunkX*16+X)/12D, (double) Y/12D, (double) (chunkZ*16+Z)/12D, 0.5D, 0.5D)) < 0.8) {
							...
							if(biomeNoiseLvl > 0.5) {
									...
									if(upOne == Material.AIR) {
										...
										if(heatNoiseLvl < -0.5 && random.nextInt(100) < -75*(heatNoiseLvl+0.5) && chunk.getBlockData(X, Y, Z).getMaterial() != Material.AIR) {
											...
											if(plant == Material.TALL_GRASS) {
												...
												if(heatNoiseLvl < -0.5 && (biomeNoiseLvl > 0.5 || ConfigUtil.allAether)) chunk.setBlock(X, Y+1, Z, main.getServer().createBlockData("minecraft:sweet_berry_bush[age=" + (random.nextInt(2) + 2) + "]"));
											} else {
												if((plant != Material.WITHER_ROSE) || (heatNoiseLvl < -0.5 && (biomeNoiseLvl > 0.5 || ConfigUtil.allAether))) chunk.setBlock(X, Y+1, Z, plant);
											}
										}
									} else chunk.setBlock(X, Y, Z, Material.DIRT);
								} else {
									chunk.setBlock(X, Y, Z, Material.STONE);
									if(upOne == Material.AIR && random.nextInt(100) < 36 && ConfigUtil.doAetherCaveDec) {
								...
								if(Y < yMax-1 && random.nextInt(100) < 32 && upOne == Material.AIR && chunk.getBlockData(X, Y, Z).getMaterial() == Material.END_STONE && ConfigUtil.doEndCaveDec) {
						...
						if(currentBlock == Material.AIR && chunk.getBlockData(X, Y+1, Z).getMaterial() == Material.END_STONE && random.nextInt(100) < 28 && ConfigUtil.doEndCaveDec && Y >= yMin) {
							switch(random.nextInt(3)) {
							case 0:
						...
						if(currentBlock == Material.AIR && chunk.getBlockData(X, Y+1, Z).getMaterial() == Material.STONE && random.nextInt(100) < 36 && ConfigUtil.doAetherCaveDec && Y >= yMin) {
