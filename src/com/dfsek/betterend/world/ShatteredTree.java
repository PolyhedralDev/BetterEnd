					...
					for(int z = -radius; z <= radius; z++) {
						...
						if(start.toVector().distance(position) <= radius + 0.5 && position.toLocation(start.getWorld()).getBlock().getType() == Material.END_STONE
								|| position.toLocation(start.getWorld()).getBlock()
										.isEmpty()) position.toLocation(start.getWorld()).getBlock().setType(random.nextBoolean() ? Material.OBSIDIAN : Material.BLACK_CONCRETE);
					}
				}
			}
			startR = startR - 0.175;
		...
		for(int x = -radius; x <= radius; x++) {
			for(int y = -radius; y <= radius; y++) {
				for(int z = -radius; z <= radius; z++) {
					...
					if(l.toVector().distance(position) <= radius + 0.5 && position.toLocation(l.getWorld()).getBlock().isEmpty()) position.toLocation(l.getWorld())
							.getBlock().setType(random.nextBoolean() ? Material.OBSIDIAN : Material.BLACK_CONCRETE);
				}
			}
		}
	}
